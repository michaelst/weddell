defmodule Weddell.Client.SubscriberTest do
  ...
  alias Google.Pubsub.V1.{Subscription, PullResponse, ReceivedMessage, ListSubscriptionsResponse}
  alias Weddell.{Message, Client, Client.Util, Client.Subscriber, SubscriberStubMock, SubscriptionDetails}
  ...
  describe "Subscriber.subscriptions/2" do
    ...
    test "list all subscriptions without paging", %{client: client} do
      ...
      subscription = %Subscription{
        push_config: %{push_endpoint: nil, attributes: nil},
        name: Util.full_subscription(@project, @subscription),
        topic: Util.full_topic(@project, @topic)
      }
      ...
      assert {:ok, [%SubscriptionDetails{}, %SubscriptionDetails{}]} = Subscriber.subscriptions(client)
    end
    ...
    test "list all subscriptions with paging", %{client: client} do
      ...
      |> expect(:list_subscriptions, fn
        _, %{project: ^project, page_token: ^cursor, page_size: ^max}, _ ->
          {:ok, %ListSubscriptionsResponse{subscriptions: subscriptions, next_page_token: cursor}}
      end)
    ...
    test "successfully pulls messages", %{client: client} do
      ...
      |> expect(:pull, fn
        _, %{subscription: ^subscription}, _ ->
          {:ok,
           %PullResponse{received_messages: [%ReceivedMessage{message: message}, %ReceivedMessage{message: message}]}}
      end)

      assert {:ok, [%Message{}, %Message{}]} = Subscriber.pull(client, @subscription)
    end
