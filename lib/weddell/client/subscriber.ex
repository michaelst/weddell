defmodule Weddell.Client.Subscriber do
  ...
  alias Weddell.{Message, Client, Client.Util, SubscriptionDetails}
  ...
  @spec create_subscription(Client.t(), name :: String.t(), topic :: String.t(), Client.subscription_options()) ::
          :ok | Client.error()
  def create_subscription(client, name, topic, opts \\ []) do
    ack_deadline = Keyword.get(opts, :ack_deadline_seconds, 10)

    push_config =
      case Keyword.get(opts, :push_endpoint, nil) do
        nil -> nil
  ...
  def delete_subscription(client, name) do
    request = DeleteSubscriptionRequest.new(subscription: Util.full_subscription(client.project, name))
  ...
  @spec acknowledge(Client.t(), (messages :: [Message.t()]) | (message :: Message.t()), subscription_name :: String.t()) ::
          :ok | Client.error()
