defmodule Weddell.Client.Subscriber.Stream do
  ...
  alias Google.Pubsub.V1.{Subscriber.Stub, StreamingPullRequest}
  alias Weddell.{Message, Client, Client.Util}

  @typedoc "A Pub/Sub subscriber stream"
  @opaque t :: %__MODULE__{client: Client.t(), subscription: String.t(), grpc_stream: GRPCStream.t()}
  ...
  @spec close(t) :: :ok
  def close(stream) do
    request = StreamingPullRequest.new(subscription: Util.full_subscription(stream.client.project, stream.subscription))
